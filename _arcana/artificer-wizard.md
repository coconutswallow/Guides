---
layout: doc
title: "Artificer-Wizard"
order: 17
category: "Arcana"
hide_from_nav: false
background_image: "/assets/images/the_dungeon_master_by_moulinbleu_d61u428-fullview.jpg"
permalink: /arcana/artificer-wizard/
--- 

Artificers are a key part of the world of Eberron. They illustrate the evolution of magic from a wild, unpredictable force to one that is becoming available to the masses. Magic items are part of everyday life in the Five Nations of Khorvaire; with an artificer in your party, they become part of every adventuring expedition.

### Artificer Summary

| Wizard Level | Arcane Tradition Feature |
| :---: | :--- |
| 2nd | Infuse potions, Infuse scrolls |
| 6th | Infuse weapons and armor |
| 10th | Superior artificer |
| 14th | Master artificer |

### Infuse Potions

Starting at 2nd level, you can produce magic potions. You spend 10 minutes focusing your magic on a vial of mundane water and expend a spell slot to transform it into a potion. Once you have expended a spell slot to create a potion, you cannot regain that slot until the potion is consumed or after 1 week, at which time the potion loses its effectiveness. You can create up to three potions at a time; creating a fourth potion causes the oldest currently active one to immediately lose its potency. If that potion has been consumed, its effects immediately end.

The spell slot you expend determines the type of potion you can create. See chapter 7 of the Dungeon Master’s Guide for complete rules on potions.

| Spell Slot | Potion Created |
| :---: | :--- |
| 1st | Climbing, growth, or healing |
| 2nd | Mind reading or greater healing |
| 3rd | Invisibility, superior healing, or water breathing |
| 4th | Resistance, hill giant strength |
| 5th | Fire breath, fly |

### Infuse Scrolls
At 2nd level, you can also tap into your reserves of magical energy to create spell scrolls. You can use your Arcane Recovery ability to create a scroll instead of regaining expended spell slots.
You must finish a short rest, then spend 10 minutes with parchment, quill, and ink to create a spell scroll containing one spell chosen from those you know. You must have all material components required to cast the spell and you expend the material components when producing the scroll.  Subtract the spell’s level from the total levels worth of slots you regain using Arcane Recovery. Scrolls created in this fashion cannot exceed 5th level. This reduction to your Arcane Recovery applies until you use the scroll and then finish a long rest.

Beginning at 14th level, in addtion to the scrolls created by utilizing Arcane Recovery, you may create one spell scroll of third level or lower. You can create one spell scroll of this variety at a time. Creating another causes the existing scroll to immediately crumble to dust. If that scroll has been cast, its effects immediately end.

## Infuse Weapons and Armor

Beginning at 6th level, you can produce magic weapons and armor. You spend 10 minutes focusing your magic on a mundane weapon, suit of armor, shield, or bundle of twenty pieces of ammunition, and expend a spell slot to infuse it with magical energy. The magic item retains its enhancement for 8 hours or until used (in the case of magic ammunition). You can infuse only one item at a time; if you infuse a second one, the first immediately loses its potency. Once you have expended a spell slot to create such an item, you cannot regain that slot until the item becomes nonmagical.

The spell slot you expend determines the type of weapon, armor, or shield you can create.

| Spell Slot | Item Created |
| :---: | :--- |
| 2nd | +1 ammunition (20 pieces) |
| 3rd | +1 weapon or +1 shield |
| 4th | +1 armor |
| 5th | +2 weapon or +2 ammunition (20 pieces) |
| 6th | +2 armor |

### Superior Artificer
Starting at 10th level, you can create a second magic weapon, suit of armor, shield, or bundle of ammunition using your Infuse Weapons and Armor ability. Attempting to infuse a third item causes the oldest one to immediately lose its potency. You can also create one additional potion or scroll using Infuse Potions or Infuse Scrolls.

### Master Artificer
On reaching 14th level, your mastery of arcane magic allows you to produce a variety of magic items. You can create a single item chosen from the appended table. It takes you 1 week to produce such an item, and you must rest for 1 month before using this ability to craft another item.

You can create a number of these magic items equal to your proficiency bonus; creating another causes your oldest created item to crumble immediately to dust.

#### Craftable Magic Items
* Alchemy jug
* Ankheg plate armor
* Cap of water breathing
* Cloak of the manta ray
* Conductor's baton
* Dragonbinding net
* Driftglobe
* Elemental gem
* Gem of brightness
* Ghostgrip gauntlets
* Goggles of night
* Helm of comprehending languages
* Horseshoes of the zephyr
* Howell's travelling cabin
* Immovable rod
* Lantern of revealing
* Medallion of blessings
* Mariner's armor
* Mithral armor
* Necklace of adaptation
* Replenishing aquarium ball
* Ring of swimming
* Rope of climbing
* Rusted iron ring
* Saddle of the cavalier
* Wand of magic detection
* Wand of secrets
* Wind fan

## Credits

### Eberron Revised
This release has been created by Luolang, Rexen, and Goose using Homebrewery in collaboration with the
Administrators, Lore Consultants, Rules Architects and Players of the Hawthorne Dungeons & Dragons Guild.

### Sources
Unearthed Arcana: Eberron - Mike Mearls
Wayfinder's Guide to Eberron - Keith Baker
