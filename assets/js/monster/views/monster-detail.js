...
export async function renderMonsterDetail(container, params) {
    ...
    // --- HELPER: Format Tags ---
    // Displays tags inside parentheses, e.g. "Fiend (Demon, Shapechanger)"
    const formatTags = (tags) => {
        if (!tags) return '';
        let tagArray = Array.isArray(tags) ? tags : [];
        
        // If Supabase returned it as a JSON string, parse it
        if (typeof tags === 'string') {
            try { tagArray = JSON.parse(tags); } catch (e) { console.warn("Tag parse error", e); }
        }
    ...
    // --- HELPER: Format Habitats ---
    const formatHabitats = (list) => {
        ...
        <div class="monster-detail-layout" style="${layoutStyle}">
            ...
            <div class="right-col"> 
                <blockquote class="stat-block">
                    ...
                    <p>
                        <em>
                            ${monster.size} ${monster.species} ${formatTags(monster.tags)}, 
                            ${alignmentText}
                        </em>
                    </p>
                    <hr>
                    ...
                    <div class="statblock-creator">
                       ...
                       <p style="margin: 0.2em 0;">
                           <strong>Environment:</strong> ${formatHabitats(monster.habitats)}
                       </p>
                    </div>
                </blockquote>
            </div>
